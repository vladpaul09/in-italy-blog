...
export interface Props {
  ...
  adjustScale?: boolean;
  data: Array<{ id: number; name: string; orderId: number }>;
  handler: (list: Array<number>) => void;
...
export function Sortable({
  ...
  const [items, setItems] = useState<UniqueIdentifier[]>(data.map((item) => item.orderId));
    ...
    <>
      <TopToolbar>
        ...
        <Button
          ...
          onClick={() => {
            handler(items.map((item) => data[(item as number) - 1].id));
          }}
        />
      </TopToolbar>
      <DndContext
            ...
            <Container>
              {items.map((value, index) => (
                <SortableItem
                  ...
                  handle={handle}
                  index={index}
                  name={data[(value as number) - 1].name}
                  style={getItemStyles}
              ...
              <DragOverlay adjustScale={adjustScale} dropAnimation={dropAnimation}>
                {activeId ? (
                  <Item
                    ...
                    handle={handle}
                    renderItem={renderItem}
                    name={data[(activeId as number) - 1].name}
                    wrapperStyle={wrapperStyle({
                      active: { id: activeId },
                      index: activeIndex,
                      isDragging: true,
