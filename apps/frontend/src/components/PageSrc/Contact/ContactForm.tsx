...
const ContactForm: FC<Props> = ({ staticStrings }) => {
  ...
  const onSubmit = handleSubmit(async (data: IForm) => {
    const res = await fetch(`${config.serverNameBackend}/api/website/contact-form`, {
      method: "POST",
      headers: {
    ...
    <Grid component="form" onSubmit={onSubmit} container size={{ xs: 12, lg: 8 }} spacing={2}>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <Controller
          name="email"
          control={control}
          rules={{ required: staticStrings.validationFormInputMessageRequired }}
          render={({ field: { onChange, value }, fieldState: { error } }) => (
            <TextField
              value={value}
              onChange={onChange}
      ...
      <Grid size={12}>
        <SubmitButton type="submit" variant="contained" fullWidth>
          {stripText(staticStrings.contactFormSubmit)}
        </SubmitButton>
      </Grid>
    </Grid>
  );
