...
import z from "zod";
import { EventCategorySchema, ProvinceSchema, ArticleCategorySchema } from "../schemas/websiteBase.schema";
...
import { Category, Province, Settings } from "../models";
...
const websiteProvinceBaseSchema = {
  params: z.object({
    locale: z.string(),
    regionSlug: z.string(),
...
const websiteProvincePageController: FastifyPluginAsyncZod<IBaseRoute> = async (app, options) => {
  const { routePrefix } = options;

  app.get(`${routePrefix}/:locale/:regionSlug/:provinceSlug`, { schema: websiteProvinceBaseSchema }, async (request, reply) => {
    const { regionSlug, provinceSlug, locale } = request.params;
      ...
      include: [
        {
          ...
          include: [{ association: "regionLanguages", required: false }],
        },
        { association: "provinceLanguages", required: false },
    ...
    const municipalityIds = (province.municipalities || []).map((m) => m.id);
    ...
    return reply.code(HTTP_STATUS_OK).send({
      province: {
        ...(await formatI18nProvince(province, currentLocale)),
        region: {
          ...(await formatI18nRegion(province.region!, currentLocale)),
        },
      },
...
export default websiteProvincePageController;
